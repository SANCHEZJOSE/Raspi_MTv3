...
float recolecta_Data(ADS1256 & ads24,ADS1115 & ads16,float ** data)
{
	...
	ads24.reboot_fast(ADS1256_DRATE_500SPS,ADS1256_GAIN_1);///Genera el reboot
//	printf("Reboot ok\n");
	while(i < filas){
		...
		ads24.setChannel(3); 
		data[i][3]=ads24.readChannel();  // Salida de los pines AIN4 y AIN5
		...
		if(check_timeout){
			...
			ads24.reboot_fast(ADS1256_DRATE_500SPS,ADS1256_GAIN_1);///Configuracion
...
float save_data(float **matriz,float delta,float t_0,char * name){
	...
	for (int i = 0; i < flag; i++){
	    fprintf(archivo,"%.4f\t",delta*(i+1)+t_0);
